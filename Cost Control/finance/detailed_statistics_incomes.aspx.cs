    ...
    public partial class detailed_statistics : System.Web.UI.Page
    {
        ...
        private void AllIncomesPlaceM()
        {
            ...
            foreach (var item in DB.GetIncome((string)Session["Authorization"]))
            {
                ...
                delete.Click += (sender, args) =>
                {
                    ...
                    Response.Redirect("detailed_statistics_incomes.aspx");
                };
        ...
        protected void Search_Click(object sender, EventArgs e)
        {
            ...
            if (StartDate.Text != null && EndDate.Text != null)
            {
                ...
                {
                    counter++;
                    foreach (var item in DB.GetIncome((string)Session["Authorization"], Start))
                    {
                        count++;
                        sum += item.Sum;
                ...
                {
                    foreach (var item in DB.GetIncome((string)Session["Authorization"], PeriodBeforeStart))
                    {
                        sum2 += item.Sum;
                    }
