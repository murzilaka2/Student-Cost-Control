...
namespace Cost_Control.finance
{
    public partial class edit_expenses : System.Web.UI.Page
    {
        ...
        private bool GetExpense()
        {
            ...
            {
                DataBase.DB DB = new DataBase.DB();
                foreach (var item in DB.GetConsumptionByID(id))
                {
                    Name.Text = item.Name;
        ...
        private void SaveToDb()
        {
            ...
            Consumption.Add(new Models.Consumption(id, DB.GetUserID((string)Session["Authorization"]), Server.HtmlEncode(Name.Text.Trim()), n, currency,
            d, Server.HtmlEncode(Definition.Text.Trim()), Server.HtmlEncode(Description.Text.Trim())));
            if (DB.EditConsumption(Consumption))
            {
